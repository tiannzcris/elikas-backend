...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-2">
        <a id="back-to-center-link" href="#" class="text-sm text-gray-500 hover:text-brand">&larr; Back to center info</a>
        <a href="/evacuation-centers" class="text-xs text-gray-400 hover:text-brand">All evacuation centers</a>
    </div>
    ...
    <div id="content-wrap" class="hidden mt-4 max-w-3xl">
        {{-- EC Information Board: the real DSWD/CSWDO aggregate report --
            ...
            sectoral group breakdown. "Add Evacuee" (age/sex breakdown)
            creates REAL Evacuee+Family records directly -- it is not a
            typed number reconciled against generated placeholders
            afterward, so families_now/persons_now and the age/sex table
            below are all LIVE, computed straight from those records every
            time this page loads. Only families_cumulative/persons_cumulative
            (a running total that survives someone later being removed) and
            the beneficiaries_4ps/sectoral figures (not known until a
            person's fuller details are filled in, so kept as a directly-
            reported aggregate) are still separately tracked. --}}
...
@section('scripts')
<script>
    // URL is /evacuation-centers/{id}/ec-board -- centerId is the segment
    // before "ec-board", not the last one (unlike the basic info page).
    const pathSegments = window.location.pathname.split('/').filter(Boolean);
    const centerId = pathSegments[1];
    ...
    (async () => {
        try {
            ...
            document.getElementById('back-to-center-link').href = `/evacuation-centers/${centerId}`;
