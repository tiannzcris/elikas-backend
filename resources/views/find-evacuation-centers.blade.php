...
<body class="bg-white text-gray-900 min-h-screen flex flex-col">
    ...
    <script>
        ...
        document.addEventListener('click', (e) => {
            // Some other handler already dealt with this click in-page.
    ...
    <script>
        ...
        function renderBreadcrumb(searching) {
            ...
            const parts = [currentBarangay === null
                ...
                : '<button type="button" data-goto="barangay" class="hover:text-brand hover:underline">All barangays</button>'];
        ...
        // A <button>, not an <a href="#">: this is an in-page action, and
        // any link click bubbles up to the page-wide navigation handler
        // above, which would tear down the map and leave the page stuck
        // mid-navigation (href="#" only changes the hash, so nothing loads).
        document.getElementById('drill-breadcrumb').addEventListener('click', (e) => {
            const btn = e.target.closest('[data-goto]');
            if (!btn) return;
            if (btn.dataset.goto === 'barangay') goToBarangayLevel();
        });
